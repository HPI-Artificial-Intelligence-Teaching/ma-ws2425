...
function plot_estimator_bias(;
    ...
    no_samples = 10,
    estimators = [
        xs -> sum((xs .- mean(xs)) .^ 2) / no_samples,
        xs -> sum((xs .- mean(xs)) .^ 2) / (no_samples - 1),
    ],
    n = 1000000,
)
    # generate a random sample of body heights
...
function plot_estimator_distribution(;
    ...
    true_pdf = [
        x -> pdf(Normal(20, 4 / sqrt(no_samples)), x),
        x ->
            (no_samples - 1) / 4^2 * pdf(Chisq(no_samples - 1), (no_samples - 1) / 4^2 * x),
    ],
    file_names = [
        "~/Downloads/plot_estimator_distribution_mean.svg",
        "~/Downloads/plot_estimator_distribution_variance.svg",
    ],
    n = 100000,
)
    # generate a random sample of body heights
