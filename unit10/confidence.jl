...
function plot_normal_distributions_across_means(;
    ...
    wireframe!(xs_coarse, μs_coarse, (x, μ) -> pdf(Normal(μ, σ), x))
    xlabel!(L"x")
    ylabel!(L"\mu")
...
function plot_two_sided_interval_normal_distributions(; μ = 0, σ = 1, α = 0.05)
    # temporary variables
    d = Normal(μ, σ)
...
function plot_two_sided_normal_distributions_confidence(;
    ...
    plot!(Shape(collect(zip(high_tail_x, high_tail_y))), fillcolor = :blue, alpha = 0.5)

    # plot the high-density region
    mid_tail_x = [μ_min + z * σ, μ_min - z * σ, μ_max - z * σ, μ_max + z * σ]
    mid_tail_y = [μ_min, μ_min, μ_max, μ_max]
    plot!(Shape(collect(zip(mid_tail_x, mid_tail_y))), fillcolor = :red, alpha = 0.5)
...
function plot_lower_one_sided_interval_normal_distributions(; μ = 0, σ = 1, α = 0.05)
    # temporary variables
    d = Normal(μ, σ)
...
function plot_lower_one_sided_normal_distributions_confidence(;
    ...
    # plot the confidence interval
    plot!([x, x], [μ_min, x - z * σ], color = :black, linewidth = 3, arrow = arrow(:closed))
...
function plot_upper_one_sided_interval_normal_distributions(; μ = 0, σ = 1, α = 0.05)
    # temporary variables
    d = Normal(μ, σ)
...
function plot_upper_one_sided_normal_distributions_confidence(;
    ...
    # plot the confidence interval
    plot!([x, x], [μ_max, x - z * σ], color = :black, linewidth = 3, arrow = arrow(:closed))
...
function plot_binomial_distribution(; n = 30, p = 0.1)
    d = Binomial(n, p)
...
function plot_approximate_binomial_confidence(; n = 50, α = 0.05, approx = false)
    # temporary variables
